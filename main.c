...
void cria_arquivo(int* L,double* alpha, int* tam,double** retorno){
	FILE *fp = NULL;
	char file[50];
...
int main(int argc,char *argv[ ]){
	...
	for (int j = 0; j < tam; j++){
		...
		if((j%10==0) && (j!=0)) printf("Rodou: %d vezes - (%f,%d)\n",j,alpha,L);
		//if((j%1000==0) && (j!=0)) cria_arquivo(&L,&a,&j,retorno)
	}

	cria_arquivo(&L,&a,&tam,retorno)
    return 0;
}
